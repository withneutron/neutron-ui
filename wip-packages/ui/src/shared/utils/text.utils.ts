...
/**
 * Converts punctuation to "fancy" variants
 * @param text text to be parsed
 * @param attr value of the smart_quotes="" attribute
 */
const SmartyPants = (text = "", attr: string | number = "1"): string => {
  let do_quotes = 0
  let do_backticks = 0
  let do_dashes = 0
  let do_ellipses = 0
  ...
  const tokens: Array<token> = _tokenize(text)
  let result = ""
  /**
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    if (cur_token[0] === "tag") {
      ...
      const last_char = t.substring(t.length - 1, t.length) // Remember last char of this token before processing.
      if (!in_pre) {
        t = ProcessEscapes(t)
...
/**
 * Converts quotes to "fancy" variants
 * @param text text to be parsed
 * @param attr value of the smart_quotes="" attribute
 */
const SmartQuotes = (text = "", attr: string | number = "1"): string => {
  /**
   * should we educate ``backticks'' -style quotes?
   */
  // let do_backticks:number;
...
/**
 * Converts dashes to "fancy" variants
 * @param text text to be parsed
 * @param attr value of the smart_quotes="" attribute
 */
const SmartDashes = (text = "", attr: string | number = "1"): string => {
  // reference to the subroutine to use for dash education, default to EducateDashes:
  let dash_sub_ref = EducateDashes
...
/**
 * Converts ellipses to "fancy" variants
 * @param text text to be parsed
 * @param attr value of the smart_quotes="" attribute
 */
const SmartEllipses = (text = "", attr: string | number = "1"): string => {
  if (typeof attr === "number") {
    attr = attr.toString()
  } else {
    attr = attr.replace(/\s/g, "")
...
const EducateQuotes = (str: string): string => {
   ...
   */
  str = str.replace(
    new RegExp(`(\\s|&nbsp;|--|&[mn]dash;|${dec_dashes}|&#x201[34])'(?=\\w)`, "g"),
    "$1&#8216;"
  ) // eslint-disable-line no-useless-escape
...
const _tokenize = (str: string): Array<token> => {
  ...
  while ((matched = match.exec(str))) {
    ...
    if (pos < matched.index) {
      const t: token = ["text", str.substring(pos, matched.index)]
      tokens.push(t)
    }
    const t: token = ["tag", matched.toString()]
    tokens.push(t)

    pos = match.lastIndex
  }
  if (pos < len) {
    const t: token = ["text", str.substring(pos, len)]
    tokens.push(t)
  }
...
/**
 * Converts punctuation to "fancy" variants, using UTF-8 characters
 * @param text text to be parsed
 * @param attr value of the smart_quotes="" attribute
 */
const smartypantsu = (text = "", attr: string | number = "1"): string => {
  const str: string = SmartyPants(text, attr)
...
export function getSmartypantsComponent(Comp: ReactNode): ReactNode {
  return walk((node: ReactChild) => {
    if (isValidElement(node)) {
      ...
      const children =
        typeof kids === "string"
          ? smartypantsu(kids)
          : Array.isArray(kids)
          ? Children.map(kids, (k: any) => (typeof k === "string" ? smartypantsu(k) : k))
          : kids
